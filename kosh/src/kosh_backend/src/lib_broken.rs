...
use ic_cdk::api::management_canister::schnorr::{
    SchnorrAlgorithm, SchnorrKeyId, SchnorrPublicKeyRequest, SchnorrSignatureRequest,
};
use serde_json;
use std::str::FromStr;
...
use stellar_xdr::curr::{
    ...
    Transaction, TransactionExt, Uint256, Uint32, WriteXdr, ReadXdr, Asset, Int64,
    DecoratedSignature, Signature, SignatureHint, TransactionEnvelope, 
...
async fn public_key_stellar() -> Result<String, String> {
    ...
    let (res,) = ic_cdk::api::management_canister::schnorr::schnorr_public_key(request)
        .await
...
async fn sign_transaction_stellar(xdr_base64: String, network: String) -> Result<String, String> {
    ...
    let (response,) = ic_cdk::api::management_canister::schnorr::sign_with_schnorr(request)
        .await
...
async fn get_sequence_number(public_key: &str, network: String) -> Result<i64, String> {
...
async fn build_stellar_transaction(
    ...
    // Get sequence number
    let sequence = get_sequence_number(&source_address, network.to_string()).await?;
    ...
    let signed_xdr = sign_transaction_stellar(xdr_base64, network.to_string()).await?;
    
    // Submit to Stellar network
    let result = submit_transaction(signed_xdr, network.to_string()).await?;
    
    Ok(result)
}

// Core function: Submit signed transactions to Stellar network
async fn submit_transaction(signed_xdr: String, network: String) -> Result<String, String> {
